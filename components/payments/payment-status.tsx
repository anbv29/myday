...
export function PaymentStatus({ intentId, accessKey }: { intentId: string; accessKey: string }) {
  ...
  if (intent.status === 'completed') return <div className="payment-state success"><p className="eyebrow">Authoritatively confirmed</p><h1>THE DAY<br />IS YOURS.</h1><p>{intent.title} · {formatMoney(intent.amountMinor, intent.currency)}</p><a className="button button-primary" href={`/day/${intent.date}`}>View the public record ↗</a></div>;
  if (intent.status === 'refunded') return <div className="payment-state"><p className="eyebrow">Date state changed</p><h1>PAYMENT<br />REFUNDED.</h1><p>Another verified claim completed first. Your payment was refunded automatically.</p><a className="button" href={`/claim?date=${intent.date}`}>See the latest price</a></div>;
  ...
  if (intent.status === 'failed' || intent.status === 'expired') return <div className="payment-state"><p className="eyebrow">Checkout ended</p><h1>NOT<br />CLAIMED.</h1><p>No ownership change was recorded.</p><a className="button button-primary" href={`/claim?date=${intent.date}`}>Start a new checkout</a></div>;
