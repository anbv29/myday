    ...
    <div className="claim-grid">
      {claims.map((claim, index) => (
        <article className="claim-card" key={claim.claimId}>
          <div className="claim-card-index"><span>{String(index + 1).padStart(2, '0')}</span><a href={`/day/${claim.isoDate}`} aria-label={`View ${claim.fullDate}`}>↗</a></div>
          <a className="claim-card-date" href={`/day/${claim.isoDate}`}>
          ...
          <div className="claim-card-copy">
            ...
            <p><a href={`/day/${claim.isoDate}`}>{claim.title}</a></p>
